        ...
        <div className="w-full space-y-5 pb-10 text-left relative">
            ...
            {loading ? <div className="text-center py-20 text-[10px] font-black uppercase text-slate-400 animate-pulse">Cargando parámetros...</div> : tenants.length === 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl">
                    <div className="p-6 rounded-2xl border border-slate-100 dark:bg-[#0B0F19] dark:border-white/5 bg-white shadow-sm space-y-6">
                        ...
                        <form onSubmit={handleSave} className="space-y-5">
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Recinto</label><select value={formData.tenantId} onChange={e => setFormData({ ...formData, tenantId: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-black uppercase outline-none focus:border-teal-500">{tenants.map(t => <option key={t.id} value={t.id} className="dark:bg-black">{t.name}</option>)}</select></div>
                                <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Cancha</label><select required value={formData.courtId} onChange={e => setFormData({ ...formData, courtId: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-black uppercase outline-none focus:border-teal-500 disabled:opacity-50" disabled={courts.length === 0}>{courts.length === 0 ? <option value="">Sin canchas</option> : null}{courts.map(c => <option key={c.id} value={c.id} className="dark:bg-black">{c.name}</option>)}</select></div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Fecha</label><input required type="date" value={formData.date} onChange={e => setFormData({ ...formData, date: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-black outline-none focus:border-teal-500 uppercase" /></div>
                                <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Hora Inicio</label><input required type="time" value={formData.time} onChange={e => setFormData({ ...formData, time: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-black outline-none focus:border-teal-500 uppercase" /></div>
                            </div>
                            <div className="space-y-4 pt-2 border-t border-slate-50 dark:border-white/5">
                                <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Cliente</label><input required type="text" value={formData.clientName} onChange={e => setFormData({ ...formData, clientName: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-black uppercase outline-none focus:border-teal-500" placeholder="NOMBRE COMPLETO" /></div>
                                <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Teléfono</label><input type="tel" value={formData.clientPhone} onChange={e => setFormData({ ...formData, clientPhone: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-black outline-none focus:border-teal-500" placeholder="+56 9..." /></div>
                            </div>
                            <div>
                                ...
                                <div className="flex gap-3">
                                    {[['pending', 'Pendiente', 'amber'], ['paid', 'Pagado', 'emerald']].map(([val, label, color]) => (
                                        <button key={val} type="button" onClick={() => setFormData({ ...formData, paymentStatus: val })} className={`flex-1 py-3 rounded-xl border text-[10px] font-black uppercase transition-all ${formData.paymentStatus === val ? `bg-${color}-50 border-${color}-500 text-${color}-700` : 'bg-slate-50 border-slate-200 text-slate-400 dark:bg-white/5 dark:border-white/10'}`}>{label}</button>
                                    ))}
                                </div>
                            </div>
                            <button type="submit" disabled={saving || courts.length === 0} className="w-full py-4 bg-slate-900 dark:bg-teal-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">{saving ? <ArrowPathIcon className="w-4 h-4 animate-spin" /> : <CheckCircleIcon className="w-4 h-4" />} Confirmar Agendamiento</button>
                        </form>
                    </div>

                    <div className="hidden lg:flex flex-col items-center justify-center p-12 bg-teal-50/50 dark:bg-teal-900/10 rounded-3xl border border-dashed border-teal-200 dark:border-teal-900/50 text-center">
                        ...
                        <div className="bg-white dark:bg-[#0B0F19] p-5 rounded-2xl border border-teal-100 dark:border-teal-500/20 shadow-sm w-full text-left">
