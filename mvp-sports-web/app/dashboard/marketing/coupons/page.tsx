    ...
    return (
        <div className="w-full space-y-5 pb-10 text-left relative animate-fadeIn">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <div className="flex flex-col gap-1">
                        ...
                        <select 
                            ...
                            className="px-5 py-2.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-[9px] font-black uppercase tracking-wider rounded-[14px] focus:ring-1 focus:ring-pink-500 outline-none text-slate-700 dark:text-slate-200"
                        >
                            {tenants.map(t => {
            ...
            <div className="flex flex-col gap-5">
                {isMarketingLocked ? (
                    <div className="w-full bg-white dark:bg-[#0B0F19] border border-slate-200 dark:border-white/5 rounded-[14px] p-8 text-center relative overflow-hidden shadow-xl animate-fadeIn flex flex-col items-center justify-center min-h-[250px]">
                        ...
                        {loading ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                                {filteredCoupons.map((c: any) => (
                                    <div key={c.id} className="bg-white dark:bg-[#0B0F19] rounded-[14px] border border-slate-100 dark:border-white/5 shadow-sm hover:border-pink-500/30 transition-all flex flex-col group overflow-hidden">
                                        <div className="p-6 flex-1 flex flex-col">
                                            <div className="flex justify-between items-start mb-4">
                                                <div className="w-10 h-10 rounded-[14px] bg-pink-50 dark:bg-pink-500/10 text-pink-600 flex items-center justify-center border border-pink-100 dark:border-pink-500/20"><PercentBadgeIcon className="w-5 h-5" /></div>
                                        ...
                                        <div className="p-3 bg-slate-50 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/5 flex gap-2">
                                            <button onClick={() => handleOpenModal(c)} className="flex-1 py-2.5 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border border-slate-200 dark:border-white/10 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">
                                                <PencilSquareIcon className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(c.id)} className="flex-1 py-2.5 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border border-red-100 dark:border-white/10 text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fadeIn" onClick={() => setIsModalOpen(false)}>
                    <div className="bg-white dark:bg-[#0B0F19] w-full max-w-md rounded-[14px] shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSave} className="p-6 space-y-4">
                            <div>
                                ...
                                <input required type="text" value={formData.code} onChange={e => setFormData({ ...formData, code: e.target.value.toUpperCase() })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[14px] text-lg font-black tracking-widest outline-none uppercase" placeholder="EJ: PADEL2026" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Descripción</label>
                                <textarea required rows={2} value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="w-full px-4 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[14px] text-xs font-bold outline-none uppercase resize-none" placeholder="RESTRICCIONES..." />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Descuento (%)</label>
                                    <input required type="number" min="1" max="100" value={formData.discount} onChange={e => setFormData({ ...formData, discount: Number(e.target.value) })} className="w-full px-4 py-2.5 bg-pink-50 dark:bg-pink-500/5 border border-pink-200 dark:border-pink-500/20 rounded-[14px] text-xs font-black outline-none" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Límite Usos</label>
                                    <input required type="number" min="1" value={formData.limit} onChange={e => setFormData({ ...formData, limit: Number(e.target.value) })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[14px] text-xs font-black outline-none" />
                                </div>
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Monto Mínimo Compra ($)</label>
                                <input required type="number" min="0" value={formData.minimumPurchase} onChange={e => setFormData({ ...formData, minimumPurchase: Number(e.target.value) })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[14px] text-xs font-black outline-none" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Fecha Inicio</label>
                                    <input required type="date" value={formData.validFrom} onChange={e => setFormData({ ...formData, validFrom: e.target.value })} className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[14px] text-[10px] font-black outline-none uppercase" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Fecha Fin</label>
                                    <input required type="date" value={formData.validUntil} onChange={e => setFormData({ ...formData, validUntil: e.target.value })} className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[14px] text-[10px] font-black outline-none uppercase" />
                                </div>
                            </div>
                            <button 
                                type="submit" 
                                disabled={saving} 
                                className="w-full py-4 bg-emerald-600 dark:bg-emerald-500 text-white dark:text-slate-950 rounded-[14px] text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-emerald-500/20 active:scale-95 disabled:opacity-50 transition-all border border-emerald-400/20 dark:border-white/10"
                            >
