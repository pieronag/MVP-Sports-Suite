  ...
  return (
    <div className="w-full space-y-6 pb-10 text-left relative animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-white/5 pb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 bg-white dark:bg-[#0B0F19] p-1.5 rounded-xl border border-slate-200 dark:border-white/5 shadow-lg">
          <button 
            onClick={fetchLogs}
            className="flex items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-white/5 rounded-lg text-[9px] font-black uppercase text-slate-500 hover:text-slate-900 dark:hover:text-white transition-all"
          >
            <ArrowPathIcon className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} />
            Sincronizar
          </button>
          <div className="w-px h-6 bg-slate-100 dark:bg-white/10 mx-1"></div>
          <button className="flex items-center gap-2 px-4 py-2 bg-slate-900 dark:bg-emerald-500 text-white dark:text-slate-900 rounded-lg text-[9px] font-black uppercase shadow-lg shadow-emerald-500/20 active:scale-95 transition-all">
      ...
      {/* TOOLBAR */}
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-center bg-white dark:bg-[#0B0F19] p-4 rounded-2xl border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/20 dark:shadow-none">
        <div className="relative w-full lg:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/20 text-[10px] font-black uppercase outline-none focus:ring-2 ring-emerald-500/20 dark:text-white font-mono"
          />
        </div>

        <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl w-full lg:w-auto">
          {['Todos', 'Críticos', 'Fallidos'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`flex-1 lg:flex-none px-6 py-2 text-[9px] font-black uppercase rounded-lg transition-all
                    ${filter === f
                ...
                filteredLogs.map((log) => (
                  <tr key={log.id} className={`group hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors ${log.severity.toLowerCase() === 'critical' || log.severity.toLowerCase() === 'high' ? 'bg-red-500/5' : ''}`}>
                    ...
                    <td className="px-6 py-5 text-center">
                      <span className={`px-2.5 py-1 rounded-md text-[8px] font-black uppercase border-b-2 transition-all ${getSeverityStyle(log.severity)}`}>
                        {log.severity}
                      </span>
                    </td>
                    <td className="px-6 py-5">
