...
const ConfirmModal = ({ isOpen, title, message, onConfirm, onCancel, isDestructive = false }: any) => {
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn" onClick={onCancel}>
            <div className="bg-white dark:bg-[#0B0F19] rounded-xl shadow-2xl border border-slate-200 dark:border-white/10 w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex gap-3 justify-end">
                    <button onClick={onCancel} className="px-4 py-2 text-[10px] font-bold uppercase rounded-xl bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300">CANCELAR</button>
                    <button onClick={onConfirm} className={`px-4 py-2 text-[10px] font-bold uppercase rounded-xl text-white ${isDestructive ? 'bg-red-600' : 'bg-emerald-600'}`}>CONFIRMAR</button>
                </div>
            </div>
        </div>
    );
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="bg-emerald-50 dark:bg-emerald-500/10 p-4 rounded-xl border border-emerald-100 dark:border-emerald-500/20">
                        ...
                        <div className="relative">
                            ...
                            <input type={showPassword ? "text" : "password"} required value={password} onChange={e => setPassword(e.target.value)} className="w-full pl-9 pr-10 py-3 bg-white dark:bg-[#0B0F19] border border-emerald-200 dark:border-emerald-500/30 rounded-xl text-xs font-bold outline-none" placeholder="MÍNIMO 6 CARACTERES" />
                    ...
                    <div className="flex gap-3">
                        <button type="button" onClick={onClose} className="flex-1 px-4 py-3 text-[10px] font-bold uppercase rounded-xl bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300">CANCELAR</button>
                        <button type="submit" disabled={isSaving || password.length < 6} className="flex-2 px-6 py-3 text-[10px] font-bold uppercase rounded-xl text-white bg-emerald-600 disabled:opacity-50 flex items-center justify-center gap-2">
...
export default function StaffPage() {
    ...
    return (
        <div className="w-full space-y-5 pb-10 text-left relative">
            ...
            {/* TOOLBAR */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-white dark:bg-[#0B0F19] p-4 rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm">
                <div className="relative">
                    <select 
                        value={filterVenueId} 
                        onChange={(e) => setFilterVenueId(e.target.value)} 
                        className="w-full bg-slate-50 dark:bg-white/5 border-0 rounded-xl pl-3 pr-8 py-3 text-[10px] font-bold uppercase outline-none text-slate-600 dark:text-white cursor-pointer appearance-none"
                    >
                ...
                <div className="relative lg:col-span-3">
                    ...
                    <input 
                        ...
                        className="w-full pl-9 pr-4 py-3 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-xl text-[10px] font-bold outline-none uppercase placeholder:text-slate-400" 
                    />
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                    {filteredStaff.map((member) => (
                        <div key={member.id} className={`bg-white dark:bg-[#0B0F19] rounded-2xl border transition-all flex flex-col ${member.status === 'active' ? 'border-slate-100 dark:border-white/5' : 'border-red-100 opacity-70'}`}>
                            ...
                            <div className="p-3 bg-slate-50 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/5 flex gap-2 rounded-b-2xl">
                                <button onClick={() => handleOpenModal(member)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 text-slate-400 hover:text-slate-900 transition-all">
                                    <PencilSquareIcon className="w-4 h-4" />
                                </button>
                                <button onClick={() => setPasswordModal({ isOpen: true, member })} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 text-slate-400 hover:text-emerald-500 transition-all">
                                    <LockClosedIcon className="w-4 h-4" />
                                </button>
                                <button onClick={() => requestToggleStatus(member)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 text-slate-400 hover:text-blue-500 transition-all">
                                    <PowerIcon className="w-4 h-4" />
                                </button>
                                <button onClick={() => requestDelete(member.id, member.uid)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-lg border border-red-100 text-red-400 hover:bg-red-50 transition-all">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fadeIn" onClick={() => setIsModalOpen(false)}>
                    <div className="bg-white dark:bg-[#0B0F19] w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSave} className="p-6 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    ...
                                    <input type="text" required value={formData.fullName} onChange={e => setFormData({ ...formData, fullName: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold outline-none uppercase" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Correo Electrónico</label>
                                    <input type="email" required value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold outline-none" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Teléfono</label>
                                    <input type="tel" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold outline-none" />
                                </div>
                                {!editingId && (
                                    <div className="col-span-2">
                                        <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Contraseña Inicial</label>
                                        <input type="password" required value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} className="w-full px-4 py-2.5 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl text-xs font-bold outline-none" />
                                    </div>
                                )}
                            </div>
                            <div className="pt-4">
                                ...
                                <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                                    {venues.map(v => (
                                        <div key={v.id} onClick={() => handleToggleVenueSelection(v.id)} className={`p-2.5 rounded-xl border cursor-pointer transition-all flex items-center gap-2 ${formData.tenantIds.includes(v.id) ? 'border-emerald-500 bg-emerald-500/10 text-emerald-600' : 'border-slate-100 dark:border-white/10 text-slate-400'}`}>
                            ...
                            <button type="submit" disabled={isSaving} className="w-full py-3 bg-slate-900 dark:bg-emerald-500 text-white dark:text-slate-900 font-black text-[10px] uppercase tracking-widest rounded-xl shadow-xl active:scale-95 disabled:opacity-50">
