    ...
    return (
        <div className="w-full space-y-6 pb-12 animate-fadeIn relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-white/5 pb-6">
                ...
                <button onClick={() => setIsModalOpen(true)} className="px-6 py-2.5 bg-slate-950 dark:bg-indigo-600 text-white dark:text-slate-950 text-[10px] font-black uppercase rounded-[14px] shadow-xl flex items-center gap-2 active:scale-95 transition-all"><PlusIcon className="w-4 h-4" /> Nueva Clase</button>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {classes.map((c: any) => (
                        <div key={c.id} className="flex flex-col h-full bg-white dark:bg-[#0B0F19] rounded-[14px] border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/20 overflow-hidden group transition-all duration-500 hover:border-indigo-500/30">
                            <div className="p-6 flex-1 flex flex-col">
                                <div className="flex justify-between items-start mb-5">
                                    <div className="p-3 bg-indigo-50 dark:bg-indigo-500/10 rounded-[14px] border border-indigo-100 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition-transform">
                                        <AcademicCapIcon className="w-6 h-6" />
                                    </div>
                                    <span className={`px-3 py-1 rounded-[14px] text-[8px] font-black uppercase tracking-widest border backdrop-blur-md ${c.status === 'active' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
                                ...
                                <div className="space-y-2 bg-slate-50 dark:bg-white/[0.03] p-4 rounded-[14px] border border-slate-100 dark:border-white/5 mb-6">
                                ...
                                <div className="mt-auto p-2 bg-slate-50 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/5 flex gap-2 -mx-6 -mb-6 rounded-[14px]">
                                    <button onClick={() => handleOpenEdit(c)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border border-slate-200 dark:border-white/10 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all shadow-sm active:scale-95"><PencilSquareIcon className="w-4 h-4" /></button>
                                    <button onClick={() => handleToggleStatus(c.id, c.status)} className={`flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border border-slate-200 dark:border-white/10 text-slate-400 ${c.status === 'active' ? 'hover:text-amber-500' : 'hover:text-emerald-500'} transition-all shadow-sm active:scale-95`}>{c.status === 'active' ? <NoSymbolIcon className="w-4 h-4" /> : <PlayCircleIcon className="w-4 h-4" />}</button>
                                    <button onClick={() => setClassToDelete(c.id)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border border-red-100 text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all shadow-sm active:scale-95"><TrashIcon className="w-4 h-4" /></button>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fadeIn" onClick={handleCloseModal}>
                    <div onClick={e => e.stopPropagation()} className="bg-white dark:bg-[#0B0F19] w-full max-w-4xl rounded-[14px] shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-5 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-slate-50/50 dark:bg-white/[0.02]">
                            <h3 className="text-sm font-black uppercase text-black dark:text-white flex items-center gap-3"><div className="p-2 bg-indigo-500 rounded-[14px] shadow-lg shadow-indigo-500/20"><AcademicCapIcon className="w-5 h-5 text-white" /></div>{isEditing ? 'Sincronizar Clase' : 'Publicar Academia'}</h3>
                        ...
                        <form onSubmit={handleCreateOrUpdate} className="p-6 overflow-y-auto space-y-8 custom-scrollbar">
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full px-3 py-2 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-[14px] text-[9px] font-normal outline-none h-20 uppercase resize-none focus:border-indigo-500 text-black dark:text-white" />
                                    </div>
                                </div>

                                {/* COLUMNA 2: PROGRAMACIÓN */}
                                <div className="space-y-4">
                                    <div className="p-3 bg-indigo-50/30 dark:bg-indigo-500/5 rounded-[14px] border border-indigo-100 dark:border-indigo-500/10">
                                        <label className="text-[8px] font-black uppercase text-indigo-600 mb-2 block">Días de Operación</label>
                                        <div className="flex flex-wrap gap-1.5">
                                            {WEEK_DAYS.map(day => (
                                                <button key={day} type="button" onClick={() => handleDayToggle(day)} className={`px-2 py-1 rounded-[14px] text-[8px] font-black uppercase transition-all border ${formData.daysOfWeek.includes(day) ? 'bg-indigo-600 text-white border-indigo-700 shadow-md shadow-indigo-500/20' : 'bg-white dark:bg-white/5 text-slate-400 border-slate-100 dark:border-white/5 hover:border-indigo-300'}`}>{day.slice(0, 3)}</button>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-[14px] border border-slate-100 dark:border-white/10 flex flex-col items-center justify-center gap-3 h-[140px] border-dashed border-2">
                            ...
                            <button type="submit" disabled={saving} className="w-full py-4 bg-black dark:bg-indigo-600 text-white dark:text-black rounded-[14px] text-[10px] font-black uppercase tracking-[0.3em] shadow-xl active:scale-[0.98] transition-all disabled:opacity-50 mt-2 flex items-center justify-center gap-2">
                ...
                <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fadeIn" onClick={() => setClassToDelete(null)}>
                    <div className="bg-white dark:bg-[#0B0F19] w-full max-w-sm rounded-[14px] p-6 border border-slate-200 dark:border-white/10 shadow-2xl text-center" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setClassToDelete(null)} className="flex-1 py-3 text-[10px] font-black uppercase rounded-[14px] bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300">CANCELAR</button>
    ...
    return (
        <div className="flex flex-col gap-1 group w-full">
            ...
            <div className="relative">
                ...
                {type === 'select' ? (
                    <select value={value || ''} onChange={onChange} className={`w-full ${icon ? 'pl-9' : 'px-3'} pr-8 py-2 bg-white dark:bg-[#0B0F19] border border-slate-100 dark:border-white/5 rounded-[14px] text-[10px] font-normal text-black dark:text-white focus:border-indigo-500 outline-none transition-all appearance-none uppercase cursor-pointer shadow-sm`}>{children}</select>
                ) : (
