    ...
    return (
        <div className="w-full space-y-6 pb-12 animate-fadeIn relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-white/5 pb-6">
                ...
                <button onClick={() => setIsModalOpen(true)} className="px-6 py-2.5 bg-slate-950 dark:bg-emerald-600 text-white dark:text-slate-950 text-[10px] font-black uppercase rounded-xl shadow-xl flex items-center gap-2 active:scale-95 transition-all"><PlusIcon className="w-4 h-4" /> Nuevo Recinto</button>
            </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredTenants.map((t) => (
                            <div key={t.id} className={`bg-white dark:bg-[#0B0F19] rounded-3xl border transition-all flex flex-col group overflow-hidden ${t.status === 'Activo' ? 'border-slate-100 dark:border-white/5' : 'border-red-100 opacity-70 shadow-none'}`}>
                                ...
                                <div className="h-40 w-full relative overflow-hidden">
                                    ...
                                    <div className="absolute top-4 left-4 flex flex-col gap-2">
                                        <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase border shadow-lg ${t.plan.toLowerCase() === 'elite' ? 'bg-amber-500 text-black border-amber-400' : t.plan.toLowerCase() === 'pro' ? 'bg-indigo-600 text-white border-indigo-400' : 'bg-slate-900 text-white border-white/10'}`}>
                                            PLAN {t.plan.toUpperCase()}
                                        </span>
                                    </div>

                                    <div className="absolute top-4 right-4">
                                        <span className={`px-2 py-1 rounded-xl text-[8px] font-black uppercase border shadow-lg ${t.status === 'Activo' ? 'bg-emerald-500 text-black border-emerald-400' : 'bg-red-500 text-white border-red-400'}`}>
                                            {t.status}
                                        </span>
                                    </div>
                                ...
                                <div className="p-3 bg-slate-50 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/5 flex gap-2">
                                    <button onClick={() => handleOpenEdit(t)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 text-slate-400 hover:text-black dark:hover:text-white transition-all active:scale-90 shadow-sm">
                                        <PencilSquareIcon className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => toggleStatus(t.id, t.status)} className={`flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-xl border transition-all active:scale-90 shadow-sm ${t.status === 'Activo' ? 'border-red-50 text-red-400 hover:bg-red-50' : 'border-emerald-50 text-emerald-500 hover:bg-emerald-50'}`}>
                                        {t.status === 'Activo' ? <NoSymbolIcon className="w-4 h-4" /> : <CheckCircleIcon className="w-4 h-4" />}
                                    </button>
                                    <button onClick={() => setTenantToDelete(t)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-xl border border-red-100 text-red-400 hover:bg-red-50 transition-all active:scale-90 shadow-sm">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fadeIn" onClick={handleCloseModal}>
                    <div onClick={e => e.stopPropagation()} className="bg-white dark:bg-[#0B0F19] w-full max-w-5xl rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-5 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-slate-50/50 dark:bg-white/[0.02]">
                            <h3 className="text-sm font-black uppercase text-black dark:text-white flex items-center gap-3"><div className="p-2 bg-emerald-500 rounded-xl shadow-lg shadow-emerald-500/20"><BuildingOffice2Icon className="w-5 h-5 text-white" /></div>{isEditing ? 'Sincronizar Recinto' : 'Alta de Recinto'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-8 custom-scrollbar">
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-1 relative">
                                        ...
                                        <div className="relative">
                                            <input type="text" placeholder="BUSCAR POR NOMBRE O EMAIL..." value={ownerSearchTerm} onChange={(e) => { setOwnerSearchTerm(e.target.value); setShowOwnerDropdown(true); }} onFocus={() => setShowOwnerDropdown(true)} className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-100 dark:border-white/5 text-[10px] font-bold bg-white dark:bg-[#0B0F19] text-black dark:text-white focus:border-emerald-500 outline-none transition-all shadow-sm uppercase" />
                                        ...
                                        {showOwnerDropdown && ownerSearchTerm && (
                                            <div className="absolute z-50 w-full mt-12 bg-white dark:bg-[#0B0F19] border border-slate-200 dark:border-white/10 rounded-xl shadow-2xl max-h-48 overflow-y-auto no-scrollbar">
                                        ...
                                        <div className="mt-3 flex flex-col gap-2">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {newTenant.selectedOwners.map(o => (
                                                    <div key={o.id} className="flex items-center gap-2 px-2 py-1 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-500/20 rounded-lg group animate-fadeIn">
                                                        ...
                                                        <button type="button" onClick={() => handleRemoveOwner(o.id)} className="p-1 hover:bg-red-50 text-red-400 rounded-md transition-all"><XMarkIcon className="w-3 h-3" /></button>
                                                    </div>
                                                ))}
                                                {newTenant.selectedOwners.length === 0 && (
                                                    <div className="w-full py-4 border-2 border-dashed border-slate-100 dark:border-white/5 rounded-xl flex items-center justify-center">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <div className="border-2 border-dashed border-slate-100 dark:border-white/10 rounded-2xl h-32 flex flex-col items-center justify-center relative bg-slate-50 dark:bg-white/5 overflow-hidden group hover:border-emerald-500 transition-all cursor-pointer">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-2 items-end">
                                        ...
                                        <button type="button" onClick={handleGeocode} className="p-2.5 bg-slate-900 text-white rounded-xl shadow-lg active:scale-90 transition-all mb-0.5"><MapPinIcon className={`w-4 h-4 ${geoLoading ? 'animate-bounce' : ''}`} /></button>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3 bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-slate-100 dark:border-white/10">
                                ...
                                <div className="space-y-4">
                                    <div className="p-6 bg-emerald-50/30 dark:bg-emerald-500/5 rounded-2xl border border-emerald-100 dark:border-emerald-500/10 flex flex-col items-center justify-center text-center gap-3">
                                    ...
                                    <div className="p-6 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/10 flex flex-col gap-3">
                            ...
                            <button type="submit" disabled={isSubmitting || newTenant.selectedOwners.length === 0} className="w-full py-4 bg-black dark:bg-emerald-600 text-white dark:text-black rounded-xl text-[10px] font-black uppercase tracking-[0.3em] shadow-xl active:scale-[0.98] transition-all disabled:opacity-50 mt-2 flex items-center justify-center gap-2">
                ...
                <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fadeIn" onClick={() => setTenantToDelete(null)}>
                    <div className="bg-white dark:bg-[#0B0F19] w-full max-w-sm rounded-2xl p-6 border border-slate-200 dark:border-white/10 shadow-2xl text-center" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setTenantToDelete(null)} className="flex-1 py-3 text-[10px] font-black uppercase rounded-xl bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300">CANCELAR</button>
                            <button onClick={handleDeleteTenant} className="flex-1 py-3 text-[10px] font-black uppercase rounded-xl text-white bg-red-600 shadow-xl shadow-red-600/20">ELIMINAR</button>
                        </div>
                    </div>
                </div>
            )}
