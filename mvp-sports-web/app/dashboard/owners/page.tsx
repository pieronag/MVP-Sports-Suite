...
export default function Page() {
    ...
    return (
        <div className="w-full space-y-6 pb-12 animate-fadeIn relative">
            {notification && (
                <div className={`fixed top-6 right-6 z-[250] flex items-center gap-3 px-5 py-3 rounded-[14px] shadow-2xl border animate-slideInRight backdrop-blur-md ${notification.type === 'success' ? 'bg-white/90 border-emerald-500 text-emerald-700 dark:bg-[#0B0F19]/90 dark:text-emerald-400 dark:border-emerald-500/50' : 'bg-white/90 border-red-500 text-red-700 dark:bg-[#0B0F19]/90 dark:text-red-400 dark:border-red-500/50'}`}>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-white/5 pb-6">
                ...
                <button onClick={() => setIsModalOpen(true)} className="px-6 py-2.5 bg-slate-950 dark:bg-emerald-600 text-white dark:text-slate-950 text-[10px] font-black uppercase rounded-[14px] shadow-xl flex items-center gap-2 active:scale-95 transition-all"><PlusIcon className="w-4 h-4" /> Nuevo Partner</button>
            </div>
            ...
            <PanelGlass className="flex flex-col md:flex-row gap-4 justify-between items-center py-3 px-4">
                ...
                <div className="flex bg-slate-50 dark:bg-white/5 p-1 rounded-[14px] border border-slate-100 dark:border-white/10">
                    {['Todos', 'Morosos', 'Suspendidos'].map(f => (
                        <button key={f} onClick={() => setFilter(f)} className={`px-4 py-1.5 text-[9px] font-black uppercase rounded-[14px] transition-all ${filter === f ? 'bg-white dark:bg-emerald-500 text-black shadow-sm' : 'text-slate-400 hover:text-black dark:hover:text-white'}`}>{f.toUpperCase()}</button>
                    ))}
                </div>
            </PanelGlass>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {filteredOwners.map((owner) => (
                        <div key={owner.id} className={`bg-white dark:bg-[#0B0F19] rounded-[14px] border transition-all flex flex-col group overflow-hidden ${owner.status === 'Activo' ? 'border-slate-100 dark:border-white/5' : 'border-red-100 opacity-70 shadow-none'}`}>
                            <div className="p-6 flex-1">
                                <div className="flex justify-between items-start mb-6">
                                    {owner.photoURL ? (
                                        <div className="w-12 h-12 rounded-[14px] border border-slate-100 dark:border-white/10 overflow-hidden shadow-inner shrink-0">
                                            <img src={owner.photoURL} className="w-full h-full object-cover" alt={owner.name} />
                                        </div>
                                    ) : (
                                        <div className="w-12 h-12 rounded-[14px] bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 flex items-center justify-center font-black text-emerald-500 text-xl shadow-inner shrink-0">{owner.name.charAt(0)}</div>
                                    )}
                                    <div className="flex flex-col items-end gap-1">
                                        <span className={`px-2.5 py-1 rounded-[14px] text-[8px] font-black uppercase border ${owner.status === 'Activo' ? 'bg-emerald-50 border-emerald-500/20 text-emerald-600' : 'bg-red-50 border-red-500/20 text-red-600'}`}>
                                            {owner.status}
                                        </span>
                                        <span className="px-2 py-0.5 bg-slate-50 dark:bg-white/5 rounded-[14px] text-[7px] font-black text-slate-400 border border-slate-100 dark:border-white/10 uppercase">{owner.tier}</span>
                                    </div>
                                </div>
                                ...
                                <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-[14px] border border-slate-100 dark:border-white/10">
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {owner.venues.slice(0, 3).map((v, i) => (
                                            <span key={i} className="px-2 py-0.5 bg-white dark:bg-white/5 text-[7px] font-black uppercase text-slate-500 rounded-[14px] border border-slate-200 dark:border-white/10">{v.name}</span>
                                        ))}
                            ...
                            <div className="p-3 bg-slate-50 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/5 flex gap-2">
                                <button title="Cambiar Password" onClick={() => handleOpenPasswordModal(owner.id)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border border-slate-200 dark:border-white/10 text-slate-400 hover:text-black dark:hover:text-white transition-all active:scale-90"><KeyIcon className="w-4 h-4" /></button>
                                <button title="Editar Perfil" onClick={() => handleOpenEdit(owner)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border border-slate-200 dark:border-white/10 text-slate-400 hover:text-black dark:hover:text-white transition-all active:scale-90"><PencilSquareIcon className="w-4 h-4" /></button>
                                <button title={owner.status === 'Activo' ? 'Suspender' : 'Activar'} onClick={() => toggleSuspend(owner.id, owner.status)} className={`flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border transition-all active:scale-90 ${owner.status === 'Activo' ? 'border-red-50 text-red-400 hover:bg-red-50' : 'border-emerald-50 text-emerald-500 hover:bg-emerald-50'}`}>{owner.status === 'Activo' ? <NoSymbolIcon className="w-4 h-4" /> : <CheckCircleIcon className="w-4 h-4" />}</button>
                                <button title="Eliminar" onClick={() => setOwnerToDelete(owner)} className="flex-1 py-2 flex items-center justify-center bg-white dark:bg-white/5 rounded-[14px] border border-red-100 text-red-400 hover:bg-red-50 transition-all active:scale-90"><TrashIcon className="w-4 h-4" /></button>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/40 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-300" onClick={handleCloseModal}>
                    <div onClick={e => e.stopPropagation()} className="bg-white dark:bg-[#0B0F19] w-full max-w-4xl rounded-[14px] shadow-[0_20px_60px_rgba(0,0,0,0.3)] border border-slate-100 dark:border-white/10 overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in duration-500">
                        {/* Cabecera Premium */}
                        <div className="p-8 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-slate-50/50 dark:bg-white/[0.02] relative">
                            ...
                            <h3 className="relative z-10 text-xl font-black uppercase text-slate-800 dark:text-white flex items-center gap-4 italic tracking-tighter">
                                <div className="p-3 bg-emerald-500 rounded-[14px] shadow-xl shadow-emerald-500/30">
                                    <BuildingStorefrontIcon className="w-6 h-6 text-white" />
                                </div>
                                {isEditing ? 'SINCRONIZAR PARTNER' : 'ALTA DE PARTNER ELITE'}
                            </h3>
                            <button onClick={handleCloseModal} className="relative z-10 p-3 text-slate-300 hover:bg-white dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white rounded-[14px] transition-all shadow-sm active:scale-90">
                        ...
                        <form onSubmit={handleSubmit} className="p-10 overflow-y-auto space-y-10 custom-scrollbar relative z-10">
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div className="p-6 bg-emerald-50/30 dark:bg-emerald-500/5 rounded-[14px] border border-emerald-100 dark:border-emerald-500/10 flex flex-col items-center justify-center text-center gap-4 group">
                            ...
                            <button 
                                ...
                                className="w-full py-5 bg-slate-900 dark:bg-emerald-500 text-white dark:text-slate-900 rounded-[14px] text-[11px] font-black uppercase tracking-[0.4em] shadow-2xl active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-4"
                            >
            ...
            {isPasswordModalOpen && (
                <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-slate-900/40 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setIsPasswordModalOpen(false)}>
                    <div onClick={e => e.stopPropagation()} className="bg-white dark:bg-[#0B0F19] w-full max-w-sm rounded-[14px] p-10 border border-slate-200 dark:border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.3)] text-center relative overflow-hidden animate-in zoom-in duration-300">
                        <div className="absolute top-0 left-0 right-0 h-24 bg-gradient-to-b from-slate-50 dark:from-white/[0.03] to-transparent pointer-events-none"></div>
                        
                        <div className="relative z-10 w-20 h-20 bg-amber-500/10 rounded-[14px] flex items-center justify-center mx-auto mb-6 border border-amber-500/20 shadow-xl shadow-amber-500/10">
                            ...
                            <div className="group">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-[14px] text-xs font-black uppercase outline-none text-slate-800 dark:text-white focus:border-amber-500 transition-all shadow-inner" 
                                    />
                                </div>
                            </div>
                            
                            <div className="flex gap-4">
                                <button type="button" onClick={() => setIsPasswordModalOpen(false)} className="flex-1 py-4 text-[9px] font-black uppercase rounded-[14px] bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 transition-all">CANCELAR</button>
                                <button type="submit" disabled={isSubmitting} className="flex-1 py-4 text-[9px] font-black uppercase rounded-[14px] text-white bg-amber-600 shadow-2xl shadow-amber-600/30 flex items-center justify-center gap-3 active:scale-95 transition-all">
            ...
            {ownerToDelete && (
                <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-slate-900/40 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setOwnerToDelete(null)}>
                    <div onClick={e => e.stopPropagation()} className="bg-white dark:bg-[#0B0F19] w-full max-w-sm rounded-[14px] p-10 border border-slate-200 dark:border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.3)] text-center relative overflow-hidden animate-in zoom-in duration-300">
                        ...
                        <div className="relative z-10 w-20 h-20 bg-rose-500/10 rounded-[14px] flex items-center justify-center mx-auto mb-6 border border-rose-500/20 shadow-xl shadow-rose-500/10">
                        ...
                        <div className="relative z-10 flex gap-4">
                            <button onClick={() => setOwnerToDelete(null)} className="flex-1 py-4 text-[9px] font-black uppercase rounded-[14px] bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 transition-all">CANCELAR</button>
                            <button onClick={handleDeleteOwner} disabled={isDeleting} className="flex-1 py-4 text-[9px] font-black uppercase rounded-[14px] text-white bg-rose-600 shadow-2xl shadow-rose-600/30 active:scale-95 transition-all">
            ...
            {createdCredentials && (
                <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/90 backdrop-blur-xl animate-in fade-in duration-500">
                    <div className="bg-white dark:bg-[#0B0F19] w-full max-w-sm rounded-[14px] p-10 border-2 border-emerald-500 shadow-[0_0_80px_rgba(16,185,129,0.3)] text-center relative overflow-hidden animate-in zoom-in duration-700">
                        ...
                        <div className="relative z-10 bg-slate-50 dark:bg-white/[0.02] p-8 rounded-[14px] border border-slate-100 dark:border-white/10 text-left space-y-6 mb-10 shadow-inner">
                            <div>
                            ...
                            <div className="pt-4 border-t border-slate-100 dark:border-white/5">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <button 
                                        ...
                                        className="p-3 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 rounded-[14px] transition-all active:scale-90"
                        ...
                        <button 
                            ...
                            className="relative z-10 w-full py-5 bg-slate-900 dark:bg-emerald-500 text-white dark:text-slate-900 rounded-[14px] text-[11px] font-black uppercase tracking-[0.4em] shadow-2xl active:scale-95 transition-all"
                        >
                            CONCLUIR ALTA
                        </button>
                    </div>
